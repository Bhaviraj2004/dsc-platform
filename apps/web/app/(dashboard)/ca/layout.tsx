...
const navItems = [
  { icon: LayoutDashboard, label: 'Dashboard', href: '/ca' },
  { icon: Users, label: 'Clients', href: '/ca/clients' },
  { icon: Key, label: 'DSC Tokens', href: '/ca/dsc-tokens' },
  { icon: FileText, label: 'Filings', href: '/ca/filings' },
  { icon: ScrollText, label: 'Documents', href: '/ca/documents' },
...
export default function CALayout({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    else if (user.role !== 'CA') router.push('/client');
    ...
    <div style={{ display: 'flex', minHeight: '100vh', background: '#f8f8f8' }}>
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '4px 8px', marginBottom: 32 }}>
          ...
          <span style={{ fontSize: 14, fontWeight: 600, color: '#111', letterSpacing: '-0.2px' }}>
            DSC Platform
          </span>
        </div>
          ...
          {navItems.map((item) => {
            ...
            return (
              <a
                ...
                onMouseEnter={e => {
                  if (!isActive) {
                    (e.currentTarget as HTMLAnchorElement).style.background = '#f8f8f8';
